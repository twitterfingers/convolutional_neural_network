...
#layer 1
model.add(tf.keras.layers.Conv2D(6, (5, 5), input_shape=X.shape[1:], padding="same"))  # 6 Feature Map
...
plt.plot(fpr, tpr, label="CNN - Lenet, auc="+str(auc))
plt.plot([0, 1], [0, 1], 'r--')
...
plot_confusion_matrixx(testY, predictions, classes=class_names,
                       title='(Lenet) Confusion matrix, without normalization')
plt.show()
