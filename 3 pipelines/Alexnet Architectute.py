...
#Layer 1
# conv layer, 96 units followed by the window size
model.add(tf.keras.layers.Conv2D(96, (11, 11), input_shape=X.shape[1:], padding="valid"))
model.add(tf.keras.layers.Activation("relu"))
model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2), padding="valid"))
model.add(tf.keras.layers.BatchNormalization())

#layer 2
model.add(tf.keras.layers.Conv2D(256, (5, 5),  padding="valid"))
model.add(tf.keras.layers.Activation("relu"))
model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2), padding='valid'))
model.add(tf.keras.layers.BatchNormalization())

#layer 3
model.add(tf.keras.layers.Conv2D(384, (3, 3), padding="valid"))
model.add(tf.keras.layers.Activation("relu"))
model.add(tf.keras.layers.BatchNormalization())
...
#Saving the model
model.save('/content/drive/My Drive/Big Data Final/AlexNet.model')
...
plt.plot(fpr, tpr, label="CNN - Alexnet, auc="+str(auc))
plt.plot([0, 1], [0, 1], 'r--')
...
plot_confusion_matrixx(testY, predictions, classes=class_names, title='(Alexnet) Confusion matrix, without normalization')
plt.show()
